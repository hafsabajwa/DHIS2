...
exports.shouldRequestBeRecorded = shouldRequestBeRecorded;
exports.handleRecordedRequest = handleRecordedRequest;
exports.completeRecording = completeRecording;

var _constants = require("../lib/constants");

var _sectionsDb = require("../lib/sections-db");

// '[]' Fallback prevents error when switching from pwa enabled to disabled
const CACHEABLE_SECTION_URL_FILTER_PATTERNS = JSON.parse(process.env.REACT_APP_DHIS2_APP_PWA_CACHING_PATTERNS_TO_OMIT_FROM_CACHEABLE_SECTIONS || '[]').map(pattern => new RegExp(pattern)); // Triggered on 'START_RECORDING' message
...
function startRecording(event) {
  ...
  self.clients.get(clientId).then(client => {
    client.postMessage({
      type: _constants.swMsgs.recordingStarted
    });
  });
}
...
function shouldRequestBeRecorded({
  url,
  event
}) {
...
function handleRecordedRequest({
  request,
  event
}) {
...
function stopRecording(error, clientId) {
  ...
  if (error) {
    self.clients.get(clientId).then(client => {
      ...
      client.postMessage({
        type: _constants.swMsgs.recordingError,
        payload: {
          msg: error.message
        }
      });
...
async function requestCompletionConfirmation(clientId) {
  ...
  client.postMessage({
    type: _constants.swMsgs.confirmRecordingCompletion
  });
...
async function completeRecording(clientId) {
  try {
    ...
    if (self.dbPromise === undefined) {
      self.dbPromise = (0, _sectionsDb.openSectionsDB)();
    } // Add content to DB


    const db = await self.dbPromise;
    db.put(_sectionsDb.SECTIONS_STORE, {
    ...
    self.clients.get(clientId).then(client => {
      client.postMessage({
        type: _constants.swMsgs.recordingCompleted
      });
    });
  } catch (err) {
