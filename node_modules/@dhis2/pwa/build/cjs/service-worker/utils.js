...
exports.setUpKillSwitchServiceWorker = setUpKillSwitchServiceWorker;
exports.urlMeetsAppShellCachingCriteria = urlMeetsAppShellCachingCriteria;
exports.createDB = createDB;
exports.removeUnusedCaches = removeUnusedCaches;
exports.getClientsInfo = getClientsInfo;
exports.claimClients = claimClients;
...
function setUpKillSwitchServiceWorker() {
  ...
  self.addEventListener('activate', async () => {
    ...
    await (0, _sectionsDb.deleteSectionsDB)(); // Force refresh all windows
...
function urlMeetsAppShellCachingCriteria(url) {
...
function createDB() {
  const dbPromise = (0, _sectionsDb.openSectionsDB)();
...
async function removeUnusedCaches() {
  ...
  return Promise.all(cacheKeys.map(async key => {
    ...
    const isASavedSection = !!(await db.get(_sectionsDb.SECTIONS_STORE, key));
...
async function getClientsInfo(event) {
  const clientId = event.source.id; // Include uncontrolled clients: necessary to know if there are multiple
  ...
  self.clients.get(clientId).then(client => {
    client.postMessage({
      type: _constants.swMsgs.clientsInfo,
      payload: {
