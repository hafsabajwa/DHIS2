...
import { startRecording, completeRecording, handleRecordedRequest, shouldRequestBeRecorded } from './recording-mode';
import { urlMeetsAppShellCachingCriteria, createDB, removeUnusedCaches, setUpKillSwitchServiceWorker, getClientsInfo, claimClients } from './utils';
export function setUpServiceWorker() {
  ...
  if (PRODUCTION_ENV) {
    ...
    registerRoute(navigationRouteMatcher, navigationRouteHandler); // Handle the rest of files in the manifest

    const restOfManifest = precacheManifest.filter(e => e !== indexHtmlManifestEntry);
    precacheAndRoute(restOfManifest); // Similar to above; manifest injection from `workbox-build`
  ...
  registerRoute(shouldRequestBeRecorded, handleRecordedRequest); // If not recording, fall through to default caching strategies for app
  // shell:
  ...
  registerRoute(({
    url
  }) => PRODUCTION_ENV && urlMeetsAppShellCachingCriteria(url) && fileExtensionRegexp.test(url.pathname), new StaleWhileRevalidate({
    cacheName: 'other-assets'
  })); // Network-first caching by default

  registerRoute(({
    url
  }) => urlMeetsAppShellCachingCriteria(url), new NetworkFirst({
    cacheName: 'app-shell'
