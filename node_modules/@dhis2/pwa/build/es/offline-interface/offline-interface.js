...
export class OfflineInterface {
  constructor() {
    ...
    this.offlineEvents = new EventEmitter(); // Receives messages from service worker and forwards to event emitter
  ...
  getClientsInfo() {
    return new Promise((resolve, reject) => {
      navigator.serviceWorker.getRegistration().then(registration => {
        ...
        this.offlineEvents.once(swMsgs.clientsInfo, resolve); // Clean up potentially unused listeners eventually
  ...
  async startRecording({
    ...
    if (!this.pwaEnabled) {
      throw new Error('Offline features are not enabled. Make sure `pwa.enabled` is `true` in `d2.config.js`');
    }
    ...
    this.offlineEvents.once(swMsgs.confirmRecordingCompletion, // Confirms recording is okay to save
    () => swMessage(swMsgs.completeRecording));
    this.offlineEvents.once(swMsgs.recordingCompleted, () => {
      cleanUpListeners();
      onCompleted();
    });
    this.offlineEvents.once(swMsgs.recordingError, ({
      msg
    }) => {
  ...
  async getCachedSections() {
    if (!this.pwaEnabled) {
      throw new Error('Cannot get cached sections - PWA is not enabled in d2.config.js');
    }
  ...
  async removeSection(sectionId) {
    if (!this.pwaEnabled) {
      throw new Error('Cannot remove section - PWA is not enabled in d2.config.js');
    }
    ...
    return Promise.all([caches.delete(sectionId), !!sectionExists && db.delete(SECTIONS_STORE, sectionId).then(() => true)]).then(([cacheDeleted, dbEntryDeleted]) => cacheDeleted || dbEntryDeleted);
  }
